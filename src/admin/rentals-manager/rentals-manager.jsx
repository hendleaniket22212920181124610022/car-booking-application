...
const RentalsManager = () => {
    ...
    const groupReservationsWithSameOwner = (allReservations) => {
        return allReservations.reduce((acc, curr) => {
            const key = curr["reservationOwner"];
            if (!acc[key]) acc[key] = [];
            acc[key].push(curr);
            return acc;
        }, {});
    ...
    const handleCancelAllReservations = () => {
        // Implementation for canceling all reservations
    };

    const handleCancelUserReservations = async (owner) => {
        Swal.fire({
            title: "Do you want to cancel all reservations of this user?",
            text: "You won't be able to revert this!",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            ...
            if (result.isConfirmed) {
                ...
                Promise.all(
                    ...
                    .then(() => {
                        Swal.fire("User's All Reservations Cancelled!", "Reservations have been removed!", "success").then((result) => {
                            if (result.isConfirmed) {
                                window.location.reload();
                            }
                        });
    ...
    const handleCancelSpecificReservation = async (documentId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                deleteDoc(doc(db, "rentals", documentId))
                    .then(() => {
                        Swal.fire("Reservation Cancelled!", "Selected reservation has been removed!", "success").then((result) => {
                            if (result.isConfirmed) {
                                window.location.reload();
                            }
                        });
                                    ...
                                    <Accordion.Body>
                                        <Accordion className="my-2">
                                            {reserveGroup.map((reserveData, idx) => {
                                                const car = cars[reserveData.carId] || {};
                                                return (
                                                    <Accordion.Item key={idx} eventKey={idx}>
                                                        <Accordion.Header>
                                                            <h3>
                                                                <BsFillCarFrontFill size="2em" className="me-2" />
                                                                <span className="fs-5 fw-bold">{`${reserveData.carBrand || "Unknown"} / ${reserveData.carModel || "Unknown"}`}</span>
                                                            </h3>
                                                        </Accordion.Header>
                                                        <Accordion.Body>
                                                            <Row>
                                                                <Col xs={12} md={6}>
                                                                    {car.image ? (
                                                                        <img src={car.image} alt={`${reserveData.carBrand} / ${reserveData.carModel}`} />
                                                                    ) : (
                                                                ...
                                                                <Col xs={12} md={6}>
                                                                    <ListGroup variant="flush">
                                                                        <ListGroup.Item>
                                                                            ...
                                                                            <span className="fs-6">HP:</span> &nbsp;
                                                                            <span className="fs-5 fw-bold">{car.power || "N/A"}</span>
                                                                        </ListGroup.Item>
                                                                        <ListGroup.Item>
                                                                            <PiEngineFill size="2em" className="me-2" />
                                                                            <span className="fs-6">Engine Size:</span> &nbsp;
                                                                            <span className="fs-5 fw-bold">{car.engineSize || "N/A"}</span>
                                                                        </ListGroup.Item>
                                                                        <ListGroup.Item>
                                                                            <TbManualGearbox size="2em" className="me-2" />
                                                                            <span className="fs-6">Gear Box:</span> &nbsp;
                                                                            <span className="fs-5 fw-bold">{car.gearbox || "N/A"}</span>
                                                                        </ListGroup.Item>
                                                                        <ListGroup.Item>
                                                                            <BsCarFront size="2em" className="me-2" />
                                                                            <span className="fs-6">Body Type:</span> &nbsp;
                                                                            <span className="fs-5 fw-bold">{car.bodyType || "N/A"}</span>
                                                                        </ListGroup.Item>
                                                                        <ListGroup.Item>
                                                                            <BsFillFuelPumpFill size="2em" className="me-2" />
                                                                            <span className="fs-6">Fuel Type:</span> &nbsp;
                                                                            <span className="fs-5 fw-bold">{car.fuelType || "N/A"}</span>
                                                                        </ListGroup.Item>
                                                                    </ListGroup>
                                                                </Col>
                                                            </Row>
